...
void	ControlUSB::BeginPortWait(int vendor, int product)
{
...
IOReturn	ControlUSB::resetrPipe()
{
	...
	kr = (*mIntf)->ResetPipe(mIntf,RPIPE);
...
IOReturn	ControlUSB::resetwPipe()
{
	...
	kr = (*mIntf)->ResetPipe(mIntf,WPIPE);
...
SInt32		ControlUSB::bulkWrite(UInt8 *buf, UInt32 size)
{
	...
	kr = (*mIntf)->WritePipe(mIntf, WPIPE, &mWriteBuffer[mWriteBufferPtr], len);
	if (kr != noErr) {
...
SInt32		ControlUSB::bulkWriteAsync(UInt8 *buf, UInt32 size)
{
	...
	kr = (*mIntf)->WritePipeAsync(mIntf, WPIPE, &mWriteBuffer[mWriteBufferPtr], len, NULL, NULL);
	if (kr != noErr) {
		printf("Write error in bulkWrite.\n");
#ifdef DEBUG_PRINT
		printErr(kr);
...
SInt32		ControlUSB::bulkRead(UInt8 *buf, UInt32 size)
{
	...
	kr = (*mIntf)->ReadPipe(mIntf, RPIPE, buf, &len);
	if (kr != noErr) {
